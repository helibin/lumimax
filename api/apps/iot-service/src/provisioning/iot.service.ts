...
import { issueEmqxDeviceCertificate } from './emqx-device-certificate.util';
...
export class IotService {
  ...
  async rotateDeviceCredential(input: {
    tenantId: string;
    deviceId: string;
    deviceSn: string;
    productKey?: string;
    ...
    const requestPayload = filterEmptyRecord({
      trigger: 'admin.devices.credential.rotate',
  ...
  private async upsertDeviceCredential(input: {
    ...
    const isRotation = pickString(payload.trigger) === 'admin.devices.credential.rotate';
  ...
  private provisionEmqxDeviceCertificate(input: {
    ...
    productKey: string;
    requestId: string;
    trigger: 'admin.devices.create' | 'devices.create' | 'admin.devices.provision' | 'admin.devices.credential.rotate';
  ...
  private async provisionAwsDeviceCertificate(input: {
    ...
    const region = resolveIotRegion();
    const policyName = pickString(getEnvString('AWS_IOT_POLICY_NAME'));
    if (!region) {
      throw new Error('IOT_REGION is required for AWS IoT provisioning');
    }
    if (!policyName) {
      throw new Error('AWS_IOT_POLICY_NAME is required for AWS IoT provisioning');
    }
  ...
  private async resolveAwsIotEndpoint(
    ...
    const configuredEndpoint = pickString(getEnvString('AWS_IOT_ENDPOINT'));
  ...
  private async deleteAwsDeviceIdentity(input: {
    ...
    const region = resolveIotRegion();
    if (!region) {
      throw new Error('IOT_REGION is required for AWS IoT deletion');
    }
    const client = this.createAwsIotClient(region);
    const policyName = pickString(getEnvString('AWS_IOT_POLICY_NAME'));
