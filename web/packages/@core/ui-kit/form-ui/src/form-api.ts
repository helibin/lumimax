...
import { Store } from '@lumimax-core/shared/store';
import {
  bindMethods,
  cloneDeep,
  createMerge,
  ...
  set,
  StateHandler,
} from '@lumimax-core/shared/utils';
...
export class FormApi {
  ...
  private setValueByFieldName(values: Record<string, any>, fieldName: string, value: any) {
    const { rawKey } = resolveFieldNamePath(fieldName);
    if (rawKey) {
      values[rawKey] = value;
      return;
