...
export class InternalMqttAuthService {
  authorize(headers: Record<string, string | string[] | undefined>): void {
    const expectedToken = pickString(getEnvString('EMQX_AUTH_SECRET'));
    if (!expectedToken) {
      throw new UnauthorizedException('emqx auth secret is not configured');
    }
    ...
    if (!providedToken || !timingSafeMatch(providedToken, expectedToken)) {
      throw new UnauthorizedException('invalid emqx auth credentials');
    }
  }
}
