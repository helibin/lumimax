...
import { get, isBoolean, isFunction } from '@lumimax-core/shared/utils';
...
export default function useDependencies(getDependencies: () => FormItemDependencies | undefined) {
  ...
  const getController = (): ExtendedFormApi => {
    const controller = isRef(extendApi) ? extendApi.value.formApi : extendApi.formApi;
  ...
  watch(
    ...
    async ([_values, dependencies]) => {
      ...
      if (isFunction(componentProps)) {
        dynamicComponentProps.value = await componentProps(formValues, formApi, getController());
      }
      ...
      if (isFunction(disabled)) {
        isDisabled.value = !!(await disabled(formValues, formApi, getController()));
      } else if (isBoolean(disabled)) {
        isDisabled.value = disabled;
      }

      if (isFunction(required)) {
        isRequired.value = !!(await required(formValues, formApi, getController()));
      }
