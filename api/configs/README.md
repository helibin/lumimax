...
- `DB_URL`
- `RABBITMQ_URL`
- `CLOUD_ACCESS_KEY_ID` / `CLOUD_ACCESS_KEY_SECRET`（当对象存储或 IoT 云链路需要云凭证时）
- `IOT_RECEIVE_MODE`
- `IOT_VENDOR`
- `IOT_QUEUE_URL`（当 `IOT_RECEIVE_MODE=queue` 时）
- `IOT_ENDPOINT`（EMQX broker 或云 IoT endpoint）
- `IOT_ROOT_CA_PEM` / `IOT_ROOT_CA_KEY_PEM`（当使用 EMQX mTLS 服务端签发时）
...
- Gateway：
  `HOST`、`HTTP_PORT`、`BASE_SERVICE_GRPC_ENDPOINT`、`BIZ_SERVICE_GRPC_ENDPOINT`
- Base-service：
  `SERVICE_NAME`、`HOST`、`HTTP_PORT`、`GRPC_PORT`、`STORAGE_VENDOR`、`STORAGE_REGION`、`STORAGE_UPLOAD_TTL_SECONDS`
- Biz-service：
  `SERVICE_NAME`、`HOST`、`HTTP_PORT`、`GRPC_PORT`、`BASE_SERVICE_GRPC_ENDPOINT`、`IOT_ENDPOINT`、`IOT_QUEUE_WAIT_TIME_SECONDS`、`IOT_QUEUE_VISIBILITY_TIMEOUT`、`IOT_QUEUE_BATCH_SIZE`、`IOT_QUEUE_IDLE_DELAY_MS`、`IOT_MQTT_USERNAME`、`IOT_MQTT_PASSWORD`、`IOT_MQTT_CLIENT_CERT_PEM`、`IOT_MQTT_CLIENT_KEY_PEM`
- AI / Nutrition：
...
- 当前重构后的默认运行组合为 `gateway` + `base-service` + `biz-service`。
...
- `/ws` 已下线；仓库默认 app 仅包含 `gateway`、`base-service`、`biz-service`。
