...
承载：云 IoT **入站**（webhook / RMQ / SQS）、topic 解析与归一化、`iot_messages` outbox、**下行发布**（含退避重试）、向 biz queue 转发 `biz.iot.message.received`。

可以：

- 依赖 `@lumimax/iot-kit` 与 `packages/mq`；消费 **iot queue**（`iot.up.*`、`iot.down.publish`）。
...
Exchange 仍为 `lumimax.bus`（topic）。当前收敛为 **两条主消费队列**：

| 队列（默认名） | 消费者 | 典型 routing key |
| --- | --- | --- |
| `lumimax.q.iot.*`（iot 总线队列） | **仅** `iot-service` | `iot.up.event`、`iot.up.lifecycle`、`iot.down.publish` |
...
| 内部 | 服务 ↔ 总线 | **RabbitMQ topic** | 异步事件 | 交换机 `lumimax.bus`；**目标**双队列 **iot / biz**（§3.5）；过渡期可仍见 upstream/downstream 三分队列 |
| 设备 | Device → EMQ X / 云 IoT | MQTT over TLS（mTLS） | 设备上行 | 见《设备协议模块规范》《IoT 通讯模块规范》 |
| 桥接 | 云 IoT ↔ **iot-service** | webhook / SQS / RabbitMQ | 传输层 | `IOT_RECEIVE_MODE=mq\|callback`；biz 只消费 biz queue |
...
- **Exchange**：`lumimax.bus`（业务与 IoT 共用 topic）、`diet.event`、`retry.event`。
