...
export class IotDispatcherService {
  constructor(
    @Inject(DeviceFacade) private readonly deviceFacade: DeviceFacade,
  ...
  private async handleConnect(
    ...
    if (message.topicKind === BizIotTopicKind.CONNECT_REQ) {
      ...
      const data = await this.deviceFacade.execute<Record<string, unknown>>({
        operation: 'internal.devices.activateFromCloud',
        params: {},
        query: {},
        body: {
    ...
    if (message.event === 'status.heartbeat') {
      const data = await this.deviceFacade.execute<Record<string, unknown>>({
        operation: 'internal.devices.recordHeartbeatTelemetry',
        params: {},
        query: {},
        body: {
          ...message.payload,
  ...
  private async handleCmdRequest(
    ...
    const data = await this.deviceFacade.execute<Record<string, unknown>>({
      operation: 'internal.devices.handleCommandResult',
      params: {},
      query: {},
      body: {
        ...message.payload,
    ...
    return {
      ...
      downlink: {
        ...
        data: {
          ...
          uploadMode: pickString(data.uploadMode) ?? 'presigned_put',
          bucket: pickString(data.bucket) ?? '',
          region: pickString(data.region) ?? '',
          objectKey: pickString(data.objectKey) ?? '',
          uploadUrl: pickString(data.uploadUrl) ?? '',
          method: pickString(data.method) ?? 'PUT',
