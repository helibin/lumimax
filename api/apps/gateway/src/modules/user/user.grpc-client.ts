...
import { USER_GRPC_SERVICE } from '@lumimax/contracts';
...
export class UserGrpcClient implements OnModuleInit {
  ...
  constructor(
    @Inject(USER_GRPC_CLIENT) grpcClient: ClientGrpc,
    logger: AppLogger,
  ) {
  ...
  onModuleInit(): void {
    this.userService = this.grpcClient.getService<UserGrpcService>(USER_GRPC_SERVICE);
  }
  ...
  async execute<T>(input: {
    ...
    this.logger.debug('网关用户服务 gRPC 调用完成', {
      requestId,
      operation: input.operation,
    }, UserGrpcClient.name);
